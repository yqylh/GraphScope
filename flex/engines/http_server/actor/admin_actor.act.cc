...
seastar::future<admin_query_result> admin_actor::create_vertex(
  ...
  if (input_json.contains("vertex_request") == false ||
    ...
    return error_response(gs::StatusCode::InvalidSchema,
      ...
      for (auto& input_property : properties_array_v) {
        ...
        if (input_property == "properties") {
          ...
          for (auto& property : (*iter)["properties"]) {
            ...
            if (new_properties_map.back().find(name_string) != new_properties_map.back().end())
          ...
          input_props_v.back()[input_property] = json_to_string(*iter);
        }
      }
    }
  } catch (std::exception& e) {
  ...
  try {
    for (int i = 0; i < edge_num; i++) {
      ...
      for (auto& edge_types : edge_types_all) {
        if (json_to_string(edge_types["type_name"]) !=
            input_props_e[i]["edge_label"])
          continue;
      ...
      for (auto& vertex_types : schema_json["vertex_types"]) {
        ...
        const std::string& pk_value =
            (sod == src_or_dst::src ? input_props_e[i]["src_primary_key_value"]
                                    : input_props_e[i]["dst_primary_key_value"]);
  ...
  try {
    ...
    if (vertex_num == 1) {
      ...
      if (txnWrite.AddVertex(label_id, insert_id, insert_arr) == false) {
        txnWrite.Abort();
        throw std::runtime_error("Fail to create vertex: " + std::to_string(0) + "; All inserts are rollbacked");
      }
      for (int i = 0; i < edge_num; i++) {
        if (txnWrite.AddEdge(
          ...
          txnWrite.Abort();
          throw std::runtime_error("Fail to create edge: " + std::to_string(i) + "; All inserts are rollbacked");
        }
      }
      txnWrite.Commit();
    } else {
      ...
      for (int i = 0; i < vertex_num; i++) {
        ...
        if (txnWrite.AddVertex(label_id, insert_id, insert_arr) == false) {
          txnWrite.Abort();
          throw std::runtime_error("Fail to create vertex: " + std::to_string(i) + "; All inserts are rollbacked");
        }
      }
      for (int i = 0; i < edge_num; i++) {
        if (txnWrite.AddEdge(
...
seastar::future<admin_query_result> admin_actor::create_edge(
  ...
  for (int i = 0; i < edge_num; i++) {
    LOG(INFO) << "edge_" << i << ": " << input_props[i]["src_label"] << "("
              << input_props[i]["src_primary_key_value"] << ") -> "
              << input_props[i]["edge_label"] << " -> "
              << input_props[i]["dst_label"] << "("
              << input_props[i]["dst_primary_key_value"] << ")"
              << " properties: " << input_props[i]["property_new_value"];
  }
    ...
    for (int i = 0; i < edge_num; i++) {
        ...
        gs::PropertyType colType =
            gs::config_parsing::StringToPrimitivePropertyType(
                json_to_string(edge_types["properties"][0]["property_type"]
                                         ["primitive_type"]));
        property_new_value_any[i] = gs::ConvertStringToAny(
            input_props[i]["property_new_value"], colType);
        break;
      }
      if (!edge_label_exists_in_schema) {
        throw std::runtime_error("Edge Label not exists in schema");
      ...
      for (auto& vertex_types : schema_json["vertex_types"]) {
        if (vertex_label_exist == 2)
          break;
        bool src_dst_same = false;
        if (vertex_types["type_name"] == input_props[i]["src_label"]) {
          vertex_label_exist++;
          sod = src_or_dst::src;
          if (input_props[i]["src_label"] == input_props[i]["dst_label"]) {
            vertex_label_exist++;
            src_dst_same = true;
          }
        } else if (vertex_types["type_name"] == input_props[i]["dst_label"]) {
          vertex_label_exist++;
          sod = src_or_dst::dst;
        } else {
          continue;
        ...
        auto& pk_value_any = (sod == src_or_dst::src ? src_pk_value_any[i]
                                                     : dst_pk_value_any[i]);
        const std::string& pk_value =
            (sod == src_or_dst::src ? input_props[i]["src_primary_key_value"]
                                    : input_props[i]["dst_primary_key_value"]);
        ...
        if (src_dst_same) {
          dst_pk_value_any[i] = gs::ConvertStringToAny(
              input_props[i]["dst_primary_key_value"], primary_key_type);
        }
      }
      if (vertex_label_exist != 2) {
        throw std::runtime_error("src_label or dst_label not exists in schema");
  ...
  try {
    ...
    for (int i = 0; i < edge_num; i++) {
      src_label_id[i] =
          db.schema().get_vertex_label_id(input_props[i]["src_label"]);
      dst_label_id[i] =
          db.schema().get_vertex_label_id(input_props[i]["dst_label"]);
      edge_label_id[i] =
          db.schema().get_edge_label_id(input_props[i]["edge_label"]);
      if (txn.GetVertexIndex(src_label_id[i], src_pk_value_any[i],
                             src_vid[i]) == false ||
          txn.GetVertexIndex(dst_label_id[i], dst_pk_value_any[i],
                             dst_vid[i]) == false) {
...
seastar::future<admin_query_result> admin_actor::update_vertex(
  ...
  try {
    for (auto& input_property : properties_array) {
      ...
      if (input_property == "properties") {
        ...
        input_props[input_property] = json_to_string(*iter);
      }
    }
  } catch (std::exception& e) {
    return error_response(gs::StatusCode::InvalidSchema,
  ...
  LOG(INFO) << "Update Vertex{" << input_props["label"] << "}:" << input_props["primary_key_value"]
            << " for graph:" << graph_id;
  try {
    auto vertex_types_all = schema_json["vertex_types"];
    bool label_exists_in_schema = false;
...
seastar::future<admin_query_result> admin_actor::update_edge(
  ...
  try {
    for (auto& input_property : properties_array) {
      ...
      if (input_property == "properties") {
        ...
        input_props["property_new_value"] = json_to_string((*iter)[0]["value"]);
      } else {
        input_props[input_property] = json_to_string(*iter);
      }
    }
  ...
  LOG(INFO) << "Update Edge{" << input_props["edge_label"] << "} v{"
            << input_props["src_label"] << "}:" << input_props["src_primary_key_value"]
            << " -> v{" << input_props["dst_label"] << "}:" << input_props["dst_primary_key_value"]
            << " for graph:" << graph_id;
  try {
    auto edge_types_all = schema_json["edge_types"];
    bool edge_label_exists_in_schema = false;
...
seastar::future<admin_query_result> admin_actor::delete_vertex(
    graph_management_param&& param) {
  return error_response(gs::StatusCode::NotFound,
                        "Delete vertex is not implemented");
}

seastar::future<admin_query_result> admin_actor::delete_edge(
    graph_management_param&& param) {
  return error_response(gs::StatusCode::NotFound,
                        "Delete edge is not implemented");
}
...
seastar::future<admin_query_result> admin_actor::error_response(
    gs::StatusCode error_code, std::string error_message) {
  return seastar::make_ready_future<admin_query_result>(
